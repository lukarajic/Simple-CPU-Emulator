...
void CPU::clock() {
    ...
    if (mem_wb_reg.valid && mem_wb_reg.controls.reg_write && mem_wb_reg.rd != 0) {
        ...
        if (mem_wb_reg.rd == id_ex_reg.rs1 && !(ex_mem_reg.valid && ex_mem_reg.controls.reg_write && ex_mem_reg.rd == id_ex_reg.rs1)) {
            op1 = wb_data;
        }
        if (mem_wb_reg.rd == id_ex_reg.rs2 && !(ex_mem_reg.valid && ex_mem_reg.controls.reg_write && ex_mem_reg.rd == id_ex_reg.rs2)) {
            op2 = wb_data;
        }
    }
    ...
    if (id_ex_reg.valid) {
        if (id_ex_reg.controls.jump) {
            ...
            if (f3 == 0) { // ECALL or MRET
                ...
                if (id_ex_reg.rd != 0) next_ex_mem.alu_result = t; 
    ...
    if (if_id_reg.valid && next_id_ex.valid && next_id_ex.controls.mem_read && (next_id_ex.rd == rs1 || next_id_ex.rd == rs2) && next_id_ex.rd != 0) {
        stall = true;
        next_id_ex = {}; 
    } else {
        stall = false;
    ...
    if (!stall) {
        ...
        next_if_id.pc = pc;
        next_if_id.valid = true; // Fetched instruction is valid
        sequential_pc = pc + 4;
    }
